...
interface PDFControlsPanelProps {
  ...
  originalPdfInfo: { width: number; height: number; } | null;
  processPdf: () => void;
  isProcessing: boolean;
}

const PDFControlsPanel: React.FC<PDFControlsPanelProps> = ({
  settings,
  setSettings,
  originalPdfInfo,
  processPdf,
  isProcessing,
    ...
    const handleDimensionChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        if (!originalPdfInfo) return;
    
        const { name, value } = e.target;
        const numValue = parseInt(value, 10) || 0;
        
        if (name === 'width' && settings.keepAspectRatio) {
          const aspectRatio = originalPdfInfo.height / originalPdfInfo.width;
          setSettings(prev => ({
            ...prev,
            width: numValue,
          ...
          const aspectRatio = originalPdfInfo.width / originalPdfInfo.height;
          setSettings(prev => ({
            ...prev,
            height: numValue,
  ...
  return (
    <div className="bg-slate-800 p-6 rounded-lg shadow-lg border border-slate-700 space-y-6">
      ...
      {/* Dimensions */}
      <div>
        <label className="block text-sm font-medium text-gray-300 mb-1">Max Dimensions (px)</label>
      ...
      <div>
        <label htmlFor="quality" className="block text-sm font-medium text-gray-300 mb-1">
            Compression Quality <span className="text-gray-400 text-xs">(Lower is smaller)</span>
        </label>
        <div className="flex items-center space-x-2">
          <input
            id="quality"
